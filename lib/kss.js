...
var walk = require('./walk.js'),
	...
	commentExpressions = {
		single: /\s*?(?:\/\/|(^|[^#])#(?!#))(.*?)$/g, // crazy because this must _not_ match '### text', but must match '# text'
		multiStart: /(?:\/\*+\!?|###)(.*?)$/,
		multiFinish: /\*\/|###/,
		multiBeforeFinish: /(.*?)(?:\*+\/|###)/
	};
...
findBlocks = function(input, options) {
	...
	for (i = 0; i < l; i += 1) {
		...
		isSingle = !insideMultiBlock && line.match(commentExpressions.single);
		isMultiStart = line.match(commentExpressions.multiStart);

		// Multi-line parsing
		if (!insideSingleBlock && !insideMultiBlock && isMultiStart) {
